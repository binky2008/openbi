package org.openbusinessintelligence.load.bodi;
...
public class CreateBodiXMLBean {

	private final static java.util.logging.Logger LOGGER = java.util.logging.Logger.getLogger(CreateBodiXMLBean.class.getPackage().getName());
		...
		for (StageObject object:objects) {
			if (stageObjectName == null || stageObjectName.equals("") || object.getEtlStageObjectName().equalsIgnoreCase(stageObjectName)) {
				...
				if (isAbap) {
					...
					BodiTableBean sourceTable = new BodiTableBean();
					...
					BodiTableBean targetTable = new BodiTableBean();
