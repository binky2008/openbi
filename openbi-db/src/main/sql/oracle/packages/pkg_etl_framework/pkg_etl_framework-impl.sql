CREATE OR REPLACE PACKAGE BODY pkg_etl_framework
AS
   /**
    * $Author: sfortis $
    * $Date: 2011-11-24 15:32:32 +0100 (Do, 24 Nov 2011) $
    * $Revision: 1857 $
    * $Id: pkg_etl_framework-impl.sql 1857 2011-11-24 14:32:32Z sfortis $
    * $HeadURL: svn://qwp1466/svn_repository_bic/edwh/dwso/edwh_adm/packages/pkg_etl_framework/pkg_etl_framework-impl.sql $
    */
   ...
   BEGIN
      EXECUTE IMMEDIATE    'select /*+ parallel (a 5) */ nvl(max(di_gui)-1,-999), nvl(max(valid_from),to_date( ''11.11.1111'',''DD.MM.YYYY'')) 
                       from '
                        || c_core_user
                        || '.'
                        || p_table_name
...
---------------------------------------------------
   ...
   BEGIN
      ...
      ELSE
          EXECUTE IMMEDIATE 'SELECT ' || p_sequence_name || ' from  dual' INTO l_seq_num;
		  -- Zuweiseung per := ist langsames als per SELECT
          -- EXECUTE IMMEDIATE 'begin :1 := ' || p_sequence_name || '; end;' USING OUT l_seq_num;
         RETURN l_seq_num;
      END IF;
   END get_sequence;
...
l_sql := '
MERGE INTO '|| l_table_name_b || ' old_rows USING
(
  SELECT  /*+ use_hash (src core)*/
  CASE
    WHEN core.'|| p_src_desc || '_B_ID IS NULL
    THEN -9999
    ELSE core.'|| p_src_desc || '_B_ID 
  END AS B_ID ,
  src.src_id,
  src.nk1,
  src.nk2
    FROM
      (
         SELECT nk1,nk2,MAX(src_id) AS src_id FROM '|| l_table_name_src|| '
         GROUP BY nk1,nk2
      ) src,
      '
         || l_table_name_b||' core
    WHERE 
	      src.nk1 = core.'|| p_nk_column_name_b1|| ' (+)
    AND src.nk2 = core.'|| l_nk_column_name_b2|| ' (+)
    AND src.src_id <> -1
    AND
      -- new and changed rows
      ( 
        DECODE(src.nk1,core.'|| p_nk_column_name_b1||',0,1)        = 1
        OR DECODE(src.nk2 ,core.'|| l_nk_column_name_b2|| ',0,1)   = 1
        OR DECODE(src.src_id ,core.'|| p_id_column_name_b|| ',0,1) = 1 
      )
) new_rows 
ON (old_rows.'|| p_src_desc|| '_B_ID = new_rows.B_ID)
WHEN MATCHED THEN
  UPDATE
  SET 
    old_rows.'|| p_id_column_name_b|| ' = new_rows.src_id,
    old_rows.DI_GUI = '|| l_di_gui || ' ,
...
l_sql := '
INSERT  /*+ append */ INTO '|| l_table_name_r|| '
  ('|| l_src_desc
    || '_R_SK ,'
    || p_sk_column_name_r|| ','
    || l_src_desc
...
from '
  || l_table_name_b|| ' SRC ,
  (SELECT '|| l_src_desc|| '_B_ID FROM '|| l_table_name_r || ' ) TGT
WHERE 
SRC.'|| l_src_desc|| '_B_ID = TGT.'|| l_src_desc|| '_B_ID (+)
AND TGT.'|| l_src_desc|| '_B_ID  IS NULL';
...
l_sql:='
INSERT  /*+ append*/ INTO '|| l_table_name_cls|| '
  (
    OLD_R_SK,
    TECH_OPERATION,
    TECH_INS_DT
...
FROM
  (select /*+ use_hash (R B) */
    R.' || l_src_desc|| '_R_SK AS OLD_R_SK,
    R.VALID_FROM,
    B.'|| p_nk_column_name_b1|| ' AS NK1,
    B.'|| l_nk_column_name_b2|| ' AS NK2
  FROM  '|| l_table_name_r|| ' R,
        '|| l_table_name_b|| ' B
  WHERE R.'|| l_src_desc|| '_B_ID  = B.'|| l_src_desc|| '_B_ID
  AND R.VALID_FROM  <> '||l_date_first||'
  AND R.VALID_FROM  <> '||l_date_dummy||'
  ) DWH,
  (SELECT NK1,NK2,TRUNC(VALID_FROM) AS VALID_FROM
  FROM ' || l_table_name_src || '
...
SELECT /*+ USE_HASH (OLD_R_TABLE NEW_R_TABLE)*/
    NVL(OLD_R_TABLE.'|| l_src_desc|| '_R_SK,-99999) as OLD_R_SK,
    OLD_R_TABLE.'|| p_sk_column_name_r|| ' as OLD_SK,
    ...
    CASE 
      WHEN OLD_R_TABLE.'|| l_src_desc|| '_R_SK is null then ''I''    
      WHEN NEW_R_TABLE.VALID_TO <> OLD_R_TABLE.VALID_TO and OLD_R_TABLE.VALID_TO = '|| l_date_last|| ' then ''V''
      WHEN NEW_R_TABLE.VALID_TO <> OLD_R_TABLE.VALID_TO and OLD_R_TABLE.VALID_TO <> '|| l_date_last|| ' then ''UC''
      WHEN NEW_R_TABLE.src_sk <> OLD_R_TABLE.'|| p_sk_column_name_r|| ' and OLD_R_TABLE.'|| p_sk_column_name_r||' <> -1 then ''UC''      
    ELSE ''US''
...
SELECT
  ...
  END  AS VALID_FROM,
  NVL(orig.VALID_TO,'|| l_date_last|| ') AS VALID_TO,
  b.'|| l_src_desc|| '_B_ID as B_ID,
  orig.LAST_DI_GUI
FROM
  (
    SELECT 
   ...
   WHERE 
    ...
    AND LAST_DI_GUI > '|| l_last_di_gui_src_imported|| '  --> delta load
  ) orig,
  '|| l_table_name_b|| ' b
WHERE 
    ORIG.nk1 = B.' || p_nk_column_name_b1|| ' AND ORIG.nk2 = B.'|| l_nk_column_name_b2|| '
AND b.'|| l_src_desc|| '_B_ID <> -1
) NEW_R_TABLE
,'|| l_table_name_r|| ' OLD_R_TABLE
WHERE 
  NEW_R_TABLE.B_ID = OLD_R_TABLE.'|| l_src_desc|| '_B_ID(+)
  ...
  ( 
   ...
   OR 
   DECODE(NEW_R_TABLE.src_sk,OLD_R_TABLE.'|| p_sk_column_name_r || ',0,1) = 1  -- Change SK
  )';
...
l_sql:=
'MERGE INTO '|| l_table_name_r|| ' TGT USING
(SELECT 
  OLD_R_SK ,
  NEW_SK,
  NEW_VALID_TO,
  ...
  NEW_VALID_FROM,
  CASE
    WHEN NEW_VALID_TO = '|| l_date_last|| '
    THEN 1
    ELSE 0
  END AS NEW_ACTIVE,
  '|| l_di_gui|| ' AS DI_GUI
FROM '|| l_table_name_cls || '
WHERE TECH_OPERATION NOT IN (''I'',''D'')
) SRC ON (tgt.'|| l_src_desc|| '_R_SK = src.OLD_R_SK)
WHEN matched THEN
  UPDATE
  SET 
    ACTIVE        = SRC.NEW_ACTIVE ,
    VALID_TO      = SRC.NEW_VALID_TO,
    '|| p_sk_column_name_r|| ' = SRC.NEW_SK,
    DI_GUI        = SRC.DI_GUI';
...
FROM 
  ...
  '|| l_table_name_b ||' b
WHERE 
    a.src_sk               = r.'||p_sk_column_name_r||' (+)
AND a.nk1                  = b.'||p_nk_column_name_b1||'
AND a.nk2                  = b.'||l_nk_column_name_b2||'
AND r.'||p_sk_column_name_r||' IS NULL';
...
BEGIN
   ...
   c_body_version    := '$Id: pkg_etl_framework-impl.sql 1857 2011-11-24 14:32:32Z sfortis $';
   c_body_url        := '$HeadURL: svn://qwp1466/svn_repository_bic/edwh/dwso/edwh_adm/packages/pkg_etl_framework/pkg_etl_framework-impl.sql $';
