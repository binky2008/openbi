CREATE OR REPLACE PACKAGE BODY stg_ddl
AS
   /**
    * $Author: nmarangoni $
    * $Date: 2012-07-24 16:51:51 +0200 (Di, 24 Jul 2012) $
    * $Revision: 3028 $
    * $Id: stg_ddl-impl.sql 3028 2012-07-24 14:51:51Z nmarangoni $
    * $HeadURL: svn://qwd4067/svn_repository_bic/edwh/dwso/edwh_adm/packages/stg_ddl/stg_ddl-impl.sql $
    */
   /*****
   ...
   c_sql_initialize                 CLOB
      := '
		trc.set_workflow_name(''#pkgName#.#prcName#'');
		l_n_result    :=
   ...
   c_sql_exception                  CLOB
      := 'stg_stat.prc_stat_end(l_n_stat_id, 0, 1);
         l_n_result    :=
   ...
   c_sql_stg1_body_incr         CLOB
      := '
        ...
        ';
   -- Insert token of the staging 1 procedure
   c_sql_stg1_body_insert       CLOB
      := 'l_n_stat_id := stg_stat.prc_stat_begin(''#sourceCode#'', ''#objectName#'', 1, #partition#, ''INS'');
   ...
   c_sql_stg1_body_stats_stg1   CLOB
      := '
        l_n_stat_id := stg_stat.prc_stat_begin(''#sourceCode#'', ''#objectName#'', 1, NULL, ''ANL'');
...
';
   ...
   c_sql_stg2_body_stats        CLOB
      := '
		...
		stg_stat.prc_size_store(''#sourceCode#'', ''#sourceTable#'', ''#tableNameStage2#'');
   ...
   l_vc_uk_column_name              type.vc_max_plsql;
   l_vc_uk_sequence_name            type.vc_max_plsql;
   -- Anonymization
   ...
   BEGIN
      ddl.prc_set_text_param (p_vc_code_string
                                    , 'columnExecIdUpd'
                                    , stg_param.c_vc_column_exec_id_upd
                                     );
      ddl.prc_set_text_param (p_vc_code_string
                                    , 'columnExecIdIns'
                                    , stg_param.c_vc_column_exec_id_ins
                                     );
      ddl.prc_set_text_param (p_vc_code_string
                                    , 'columnTimestamp'
                                    , stg_param.c_vc_column_timestamp
                                     );
      ddl.prc_set_text_param (p_vc_code_string
                                    , 'columnDmlOperation'
                                    , stg_param.c_vc_column_dml_op
      ...
      ddl.prc_set_text_param (p_vc_code_string
                                    , 'columnPartition'
                                    , stg_param.c_vc_column_partition
                                     );
      ddl.prc_set_text_param (p_vc_code_string
                                    , 'columnActiveVersion'
                                    , stg_param.c_vc_column_active_version
                                     );
      ddl.prc_set_text_param (p_vc_code_string
                                    , 'columnValidFrom'
                                    , stg_param.c_vc_column_valid_from
                                     );
      ddl.prc_set_text_param (p_vc_code_string
                                    , 'columnValidTo'
                                    , stg_param.c_vc_column_valid_to
                                     );
      ddl.prc_set_text_param (p_vc_code_string
                                    , 'columnSourceDistribution'
                                    , stg_param.c_vc_column_source_distr
                                     );
   ...
   BEGIN
      FOR r_col IN (SELECT   table_name
                        ...
                        FROM all_tab_columns exi
                           , (SELECT col.stg_object_id
                                   , col.stg_object_name
                                   , col.stg_stg2_table_name
                                   , col.stg_column_pos
                                   , col.stg_column_def
                                   , msk.src_column_name
                                   , msk.trg_column_name
                                ...
                                FROM (SELECT o.stg_object_id
                                        ...
                                        FROM stg_object_t o
                                           , stg_column_t c
                                       WHERE o.stg_object_id = c.stg_object_id) col
                               ...
                               WHERE col.stg_stg2_table_name = msk.table_name
                                 AND col.stg_column_name = msk.src_column_name) met
                       WHERE met.stg_stg2_table_name = exi.table_name(+)
                         AND met.trg_column_name = exi.column_name(+)
                         AND exi.owner(+) = g_vc_owner_stg
                         AND exi.owner IS NULL
                    ORDER BY stg_column_pos)
      LOOP
         l_vc_def_anonymized    := l_vc_def_anonymized || ',' || r_col.trg_column_name || ' ' || r_col.stg_column_def;
         l_vc_ini_anonymized    :=
               l_vc_ini_anonymized
            || ','
                        ...
                        FROM all_tab_columns exi
                           ...
                           , (SELECT atab.table_name
                               ...
                               WHERE asch.schema_id = atab.schema_id
                                 ...
                                 AND acol.business_attribute_id = attr.attribute_id
                                 AND attr.anonym_method_id = meth.method_id) msk
                       WHERE col.stg_stg2_table_name = exi.table_name
                         AND col.stg_column_name = exi.column_name
                         AND col.stg_stg2_table_name = msk.table_name
                         AND col.stg_column_name = msk.src_column_name
                         AND col.stg_object_id = g_n_object_id
                         AND exi.owner = g_vc_owner_stg
                    ORDER BY stg_column_pos)
      LOOP
   ...
   IS
   BEGIN
      MERGE INTO stg_ddl_t trg
         USING (SELECT UPPER (p_vc_object_type) AS object_type
                     , UPPER (p_vc_object_name) AS object_name
                     , p_vc_object_ddl AS object_ddl
                  FROM DUAL) src
         ON (    UPPER (trg.stg_ddl_type) = UPPER (src.object_type)
             AND UPPER (trg.stg_ddl_name) = UPPER (src.object_name))
         WHEN MATCHED THEN
            UPDATE
               SET trg.stg_ddl_code = src.object_ddl
         WHEN NOT MATCHED THEN
            INSERT (trg.stg_ddl_type, trg.stg_ddl_name, trg.stg_ddl_code)
   ...
   BEGIN
      ...
      FOR r_comm IN (SELECT c.stg_column_name
                          , c.stg_column_comment
                       FROM stg_object_t o
                          , stg_column_t c
                      WHERE o.stg_object_id = c.stg_object_id
                        AND o.stg_object_id = g_n_object_id)
      LOOP
   ...
   BEGIN
      ...
      l_l_utl_columns        := type.fct_string_to_list (l_sql_utl_columns, ',');
   ...
   BEGIN
      ...
      -- HEAD
      --
      l_sql_prc                := ddl.c_template_prc_head;
      ddl.prc_set_text_param (l_sql_prc
                                    , 'prcName'
                                    , 'prc_trunc_stage1'
                                     );
      ddl.prc_set_text_param (l_sql_prc
      ...
      ddl.prc_set_text_param (l_sql_prc_token
                                    , 'tableName'
                                    , g_vc_table_name_stage1
                                     );
      ...
      l_sql_prc                := ddl.c_template_prc_body;
      ddl.prc_set_text_param (l_sql_prc
                                    , 'prcParameters'
                                    , c_sql_prc_param
                                     );
   ...
   BEGIN
      ...
      ddl.prc_set_text_param (l_sql_prc_buffer
                                    , 'tableName'
                                    , g_vc_table_name_diff
                                     );
      -- Put body in the generic prc template
   ...
   BEGIN
      ...
      l_vc_col_all             :=
         dict.fct_get_column_subset (g_vc_dblink
                                          , g_vc_owner_src
      ...
      l_sql_prc_token          := 'EXECUTE IMMEDIATE ''ALTER SESSION ENABLE PARALLEL DML'';' || CHR (10) || c_sql_stg2_empty;
      ddl.prc_set_text_param (l_sql_prc_token
                                    , 'tableName'
                                    , g_vc_table_name_stage2
                                     );
      ...
      LOOP
         IF     g_n_source_nk_flag = 0
            AND g_vc_col_pk_src IS NOT NULL
         THEN
            l_vc_col_pk_notnull    := stg_meta.fct_get_column_list (g_n_object_id
                                                                            , 'PK'
                                                                            , 'AND_NOTNULL'
                                                                             );
   ...
   BEGIN
      ...
      LOOP
         ...
         THEN
            ...
            l_sql_prc_buffer       := l_sql_prc_buffer || CHR (10) || c_sql_stg1_body_dedupl;
            ddl.prc_set_text_param (l_sql_prc_buffer
                                          , 'tableNameDupl'
                                          , g_vc_table_name_dupl
                                           );
         ...
         ELSE
            l_sql_prc_buffer    := l_sql_prc_buffer || CHR (10) || c_sql_stg1_body_insert;
         END IF;
   ...
   BEGIN
      ...
      l_vc_col_pk_2            := dict.fct_get_column_list (NULL
                                                                 , g_vc_owner_stg
                                                                 , g_vc_table_name_stage2
                                                                 , 'PK'
                                                                 , 'LIST_SIMPLE'
      ...
      l_vc_col_all             := dict.fct_get_column_subset (NULL
                                                                   , g_vc_owner_stg
                                                                   , g_vc_table_name_stage1
                                                                   , g_vc_owner_stg
                                                                   , g_vc_table_name_stage2
      ...
      l_sql_prc_buffer         := l_sql_prc_buffer || CHR (10) || c_sql_stg2_check;

      IF g_vc_col_pk IS NULL
      THEN
         -- If there is no natural key (tecnical PK) then use the alternate difference method
         l_vc_clause_on    := dict.fct_get_column_subset (NULL
                                                               , g_vc_owner_stg
                                                               , g_vc_table_name_stage1
                                                               , g_vc_owner_stg
                                                               , g_vc_table_name_stage2
      ...
      ELSE
         ...
         l_vc_clause_on         := dict.fct_get_column_list (NULL
                                                                  , g_vc_owner_stg
                                                                  , g_vc_table_name_stage2
                                                                  , 'PK'
                                                                  , 'AND_ALIAS'
         ...
         l_vc_upd_clause_set    := dict.fct_get_column_subset (NULL
                                                                    , g_vc_owner_stg
                                                                    , g_vc_table_name_stage1
                                                                    , g_vc_owner_stg
                                                                    , g_vc_table_name_stage2
         ...
         l_vc_col_nvl2          := dict.fct_get_column_subset (NULL
                                                                    , g_vc_owner_stg
                                                                    , g_vc_table_name_stage1
                                                                    , g_vc_owner_stg
                                                                    , g_vc_table_name_stage2
         ...
         l_vc_clause_update     := dict.fct_get_column_subset (NULL
                                                                    , g_vc_owner_stg
                                                                    , g_vc_table_name_stage1
                                                                    , g_vc_owner_stg
                                                                    , g_vc_table_name_stage2
      ...
      THEN
         ...
         LOOP
            IF g_vc_col_pk IS NULL
            THEN
               l_sql_prc_token_iter    := c_sql_stg2_body_diff_nonk || c_sql_stg2_body_stg2_2dml;
            ELSE
               l_sql_prc_token_iter    := c_sql_stg2_body_diff_nk || c_sql_stg2_body_stg2_2dml;
            END IF;

            dict.prc_set_text_param (l_sql_prc_token_iter
                                          , 'enableParallelDML'
                                          , CASE
      ...
      ELSE
         IF g_vc_col_pk IS NULL
         THEN
            l_sql_prc_token    := c_sql_stg2_body_diff_nonk || c_sql_stg2_body_stg2_2dml;
         ELSE
            l_sql_prc_token    := c_sql_stg2_body_diff_nk || c_sql_stg2_body_stg2_2dml;
         END IF;
      ...
      l_sql_prc_buffer         := l_sql_prc_buffer || CHR (10) || l_sql_prc_token || CHR (10) || c_sql_stg2_body_stats;
   ...
   BEGIN
      ...
      l_vc_ins_col_source      := dict.fct_get_column_subset (NULL
                                                                   , g_vc_owner_stg
                                                                   , g_vc_table_name_stage1
                                                                   , g_vc_owner_stg
                                                                   , g_vc_table_name_stage2
      ...
      l_vc_ins_col_target      := dict.fct_get_column_subset (NULL
                                                                   , g_vc_owner_stg
                                                                   , g_vc_table_name_stage1
                                                                   , g_vc_owner_stg
                                                                   , g_vc_table_name_stage2
      ...
      THEN
         ...
         LOOP
            ...
            ddl.prc_set_text_param (l_sql_prc_token_iter
                                          , 'enableParallelDML'
                                          , CASE
                                               WHEN l_vc_set_anonymized IS NOT NULL
                                                  THEN c_sql_enable_parallel_dml
   ...
   BEGIN
      ...
      -- BODY for FULL load
      --
      l_sql_prc_buffer         := c_sql_stg_wrapper;
   ...
   BEGIN
      ...
      ddl.prc_create_object ('PACKAGE BODY'
                                   , g_vc_package_main
                                   , l_sql_create
                                   , FALSE
                                   , p_b_raise_flag
   ...
   BEGIN
      ...
      THEN
         ...
         l_vc_col_src     := dict.fct_get_column_list (g_vc_dblink
                                                            , g_vc_owner_src
                                                            , g_vc_table_name_source
                                                            , 'ALL'
                                                            , 'LIST_SIMPLE'
                                                             );
         l_vc_col_dupl    := dict.fct_get_column_subset (g_vc_dblink
                                                              , g_vc_owner_src
                                                              , g_vc_table_name_source
                                                              , g_vc_owner_stg
                                                              , g_vc_table_name_dupl
 ...
 */
BEGIN
   c_body_version    := '$Id: stg_ddl-impl.sql 3028 2012-07-24 14:51:51Z nmarangoni $';
   c_body_url        := '$HeadURL: svn://qwd4067/svn_repository_bic/edwh/dwso/edwh_adm/packages/stg_ddl/stg_ddl-impl.sql $';
END stg_ddl;
/
...
BEGIN
   ddl.prc_create_synonym ('stg_ddl'
                         , 'stg_ddl'
                         , TRUE
                         );
END;
