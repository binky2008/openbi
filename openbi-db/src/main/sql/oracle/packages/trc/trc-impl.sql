CREATE OR REPLACE PACKAGE BODY trc AS
  ...
  */
  ...
  BEGIN
    ...
    IF g_n_console_size >= param.g_n_log_console_max * 0.88 THEN
  ...
  PROCEDURE log(p_n_severity        IN NUMBER DEFAULT param.gc_log_info
  ...
  BEGIN
    ...
    IF p_n_severity <= param.g_n_log_level THEN
      -- counter increment
      ...
      IF g_n_message_count = param.g_n_log_message_max THEN
      ...
      IF param.g_b_log_console THEN
        ...
        log_console('AUDSID: ' || param.g_vc_audsid ||
                    ' SESSION_USER: ' || param.g_vc_session_user ||
                    ' OS_USER: ' || param.g_vc_os_user ||
                    ' TERMINAL: ' || param.g_vc_terminal);
        log_console('SQLCODE: ' || l_sqlcode || ' SQLERRM: ' || l_sqlerrm);
      END IF;
    
      IF param.g_b_log_table THEN
        INSERT INTO trc_t
          (trc_severity
          ,trc_message_short
          ,trc_message_long
        ...
        VALUES
          ...
          ,l_sqlerrm
          ,CASE
             WHEN p_n_severity < param.gc_log_warn THEN
              l_call_stack
           END
          ,p_n_row_count
          ,p_n_external_job_id
          ,p_vc_text_big)
        RETURNING trc_id INTO l_trc_id;
      
        COMMIT;
      
        /*IF p_vc_text_big IS NOT NULL THEN
          INSERT INTO trc_text_t
            (log_id
            ,trc_text_big)
          VALUES
            (l_log_id
...
BEGIN
  ...
  IF param.g_b_log_console THEN
    dbms_output.enable(param.g_n_log_console_max);
  END IF;
