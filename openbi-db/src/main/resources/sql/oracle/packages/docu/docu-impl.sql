...
IS
   ...
   */
   ...
   BEGIN
      IF p_vc_stylesheet_type = 'HTML' THEN
         RETURN docu_param.c_xsl_html_table_default;
      ELSIF p_vc_stylesheet_type = 'HTML' THEN
         RETURN docu_param.c_xsl_excel_table_default;
      ELSE
         RETURN NULL;
      END IF;
   ...
   */
   ...
   BEGIN
      CASE p_vc_type
         WHEN 'html' THEN
            l_clob_document := docu_param.c_html_template_content;
            prc_set_text_param (
               l_clob_document
             , 'htmlScript'
             , docu_param.c_js_default
            );
            prc_set_text_param (
               l_clob_document
             , 'htmlStyle'
             , docu_param.c_css_default
            );
            prc_set_text_param (
               l_clob_document
             , 'htmlContent'
         ...
         WHEN 'ms-excel' THEN
            l_clob_document := docu_param.c_excel_template_content;
            prc_set_text_param (
               l_clob_document
             , 'workbookContent'
             , p_vc_content
   ...
   BEGIN
      ...
      IF p_vc_column_list IS NULL THEN
         ...
         l_vc_column_list :=
            LTRIM (
               TYPE.fct_list_to_string (l_l_columns)
             , ' ||'
            );
      ELSE
         l_l_columns :=
