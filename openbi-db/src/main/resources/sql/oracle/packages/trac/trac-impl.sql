CREATE OR REPLACE PACKAGE BODY trac
AS
   /**
   * $Author: nmarangoni $
   * $Date: $
   ...
   */
   ...
   BEGIN
      ...
      IF g_n_console_size >= trac_param.g_n_log_console_max * 0.88 THEN
   ...
   PROCEDURE LOG (
      p_n_severity          IN NUMBER DEFAULT trac_param.gc_log_info
   ...
   BEGIN
      ...
      IF p_n_severity <= trac_param.g_n_log_level THEN
         ...
         IF g_n_message_count = trac_param.g_n_log_message_max THEN
            l_message_long :=
         ...
         IF trac_param.g_b_log_console THEN
            ...
            log_console (   'OBJECT: '
                         ...
                         || l_line_nr);
            log_console (   'AUDSID: '
                         || trac_param.g_vc_audsid
                         || ' SESSION_USER: '
                         || trac_param.g_vc_session_user
                         || ' OS_USER: '
                         || trac_param.g_vc_os_user
                         || ' TERMINAL: '
                         || trac_param.g_vc_terminal);
            log_console (   'SQLCODE: '
                         || l_sqlcode
                         || ' SQLERRM: '
                         || l_sqlerrm);
         ...
         IF trac_param.g_b_log_table THEN
            INSERT INTO trac_t (
                           trac_severity
                         , trac_message_short
                         , trac_message_long
                 ...
                 VALUES (
                         ...
                         , l_sqlerrm
                         , CASE
                              WHEN p_n_severity < trac_param.gc_log_warn THEN
                                 l_call_stack
                           END
                         , p_n_row_count
                         , p_n_external_job_id
...
BEGIN
   ...
   IF trac_param.g_b_log_console THEN
      DBMS_OUTPUT.enable (trac_param.g_n_log_console_max);
   END IF;
